...
- → See `go-observability` skill for slog fundamentals (levels, context, handler setup, migration)
- → See `go-error-handling` skill for the log-or-return rule
- → See `go-security` skill for PII handling in logs
- → See `go-samber-oops` skill for structured error context with `samber/oops`
