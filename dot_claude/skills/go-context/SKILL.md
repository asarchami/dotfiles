...
> **Community default.** A company skill that explicitly supersedes `go-context` skill takes precedence.
...
- → See the `go-concurrency` skill for goroutine cancellation patterns using context
- → See the `go-database` skill for context-aware database operations (QueryContext, ExecContext)
- → See the `go-observability` skill for trace context propagation with OpenTelemetry
- → See the `go-design-patterns` skill for timeout and resilience patterns
