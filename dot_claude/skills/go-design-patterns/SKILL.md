...
> **Community default.** A company skill that explicitly supersedes `go-design-patterns` skill takes precedence.

# Go Design Patterns & Idioms

Idiomatic Go patterns for production-ready code. For error handling details see the `go-error-handling` skill; for context propagation see `go-context` skill; for struct/interface design see `go-structs-interfaces` skill.
...
→ See `go-structs-interfaces` for the `var _ Interface = (*Type)(nil)` pattern.

## Error Flow Patterns

Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `go-code-style` for the full pattern and examples.
...
Retry logic MUST check `ctx.Err()` between attempts and use exponential/linear backoff via `select` on `ctx.Done()`. Long loops MUST check `ctx.Err()` periodically. → See `go-context` skill.

## Database Patterns

→ See `go-database` skill for sqlx/pgx, transactions, nullable columns, connection pools, repository interfaces, testing.
...
- → See `go-data-structures` skill for data structure selection, internals, and container/ packages
- → See `go-error-handling` skill for error wrapping, sentinel errors, and the single handling rule
- → See `go-structs-interfaces` skill for interface design and composition
- → See `go-concurrency` skill for goroutine lifecycle and graceful shutdown
- → See `go-context` skill for timeout and cancellation patterns
