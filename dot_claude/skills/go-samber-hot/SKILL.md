...
- → See `go-performance` skill for general caching strategy and when to use in-memory cache vs Redis vs CDN
- → See `go-observability` skill for Prometheus metrics integration and monitoring
- → See `go-database` skill for database query patterns that pair with cache loaders
