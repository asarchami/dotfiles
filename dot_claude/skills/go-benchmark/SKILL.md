...
- → See `go-performance` skill for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `go-troubleshooting` skill for pprof setup on running services (enable, secure, capture), Delve debugger, GODEBUG flags, root cause methodology
- → See `go-observability` skill for everyday always-on monitoring, continuous profiling (Pyroscope), distributed tracing (OpenTelemetry)
