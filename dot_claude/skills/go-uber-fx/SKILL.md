---
name: go-uber-fx
description: "Golang application framework using uber-go/fx — fx.New, fx.Provide, fx.Invoke, fx.Module, fx.Lifecycle hooks, fx.Annotate (name/group/As), fx.Decorate, fx.Supply, fx.Replace, fx.WithLogger, and signal-aware Run(). Apply when using or adopting uber-go/fx, when the codebase imports `go.uber.org/fx`, or when wiring services with fx.New. For raw DI without lifecycle, see `go-uber-dig` skill."
user-invocable: true
license: MIT
...
**Choose raw dig** when you need wiring without a framework: CLI tools, libraries that expose a container to callers, test harnesses, or embedding DI into an existing app that manages its own lifecycle. See `go-uber-dig` skill.
...
- → See `go-uber-dig` skill for the underlying container, `dig.In`/`dig.Out`, and DI without lifecycle
...
- → See `go-context` skill for context propagation in OnStart/OnStop hooks
