...
6. **Suggest improvements contextually**:
   ...
   - If invoked explicitly via `/go-modernize` or in CI, scan and suggest across the entire codebase.
...
See `go-concurrency`, `go-testing`, `go-observability`, `go-error-handling`, `go-lint`, `go-continuous-integration` skills.
