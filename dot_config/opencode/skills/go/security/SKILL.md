...
Security-relevant linters: `bodyclose`, `sqlclosecheck`, `nilerr`, `errcheck`, `govet`, `staticcheck`. See the `go-lint` skill for configuration and usage.
...
# Vulnerability scanner — see go-dependency-management for full govulncheck usage
...
See `go-database`, `go-safety`, `go-observability`, `go-continuous-integration` skills.
