...
> **Community default.** A company skill that explicitly supersedes `go-naming` skill takes precedence.
...
Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `go-lint` skill for configuration and usage.

## Cross-References

- → See `go-code-style` skill for broader formatting and style decisions
- → See `go-structs-interfaces` skill for interface naming depth and receiver design
- → See `go-lint` skill for automated enforcement (revive, predeclared, misspell, errname)
