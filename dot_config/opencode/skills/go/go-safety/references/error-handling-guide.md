...
> **Community default.** A company skill that explicitly supersedes `go-safety` skill takes precedence.
...
- → See `samber-oops` for full samber/oops API, builder patterns, and logger integration
...
- → See `go-style` for error naming conventions (ErrNotFound, PathError)
