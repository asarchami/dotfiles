...
Idiomatic Go patterns for production-ready code. For error handling details see the `go-safety` skill; for context propagation see `go-context` skill; for struct/interface design see `go-structs-interfaces` skill.
...
12. **Use `strings.Builder`** for concatenation in loops → see `go-style`
...
Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `go-style` for the full pattern and examples.
...
- → See `go-safety` skill for error wrapping, sentinel errors, and the single handling rule
