...
> **Community default.** A company skill that explicitly supersedes `go-style` skill takes precedence.

# Go Code Style

Style rules that require human judgment — linters handle formatting, this skill handles clarity. For naming see `go-style` skill; for design patterns see `go-patterns` skill; for struct/interface design see `go-structs-interfaces` skill.
...
- Functions SHOULD have **≤4 parameters**. Beyond that, use an options struct (see `go-patterns` skill).
...
Many rules are enforced automatically: `gofmt`, `gofumpt`, `goimports`, `gocritic`, `revive`, `wsl_v5`. → See the `go-lint` skill.

## Cross-References

- → See the `go-style` skill for identifier naming conventions
- → See the `go-structs-interfaces` skill for pointer vs value receivers, interface design
- → See the `go-patterns` skill for functional options, builders, constructors
- → See the `go-lint` skill for automated formatting enforcement
