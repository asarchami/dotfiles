...
For interface-based design foundations (accept interfaces, return structs), see the `go-structs-interfaces` skill.
...
- → See `go-samber-do` skill for detailed samber/do usage patterns
- → See `go-structs-interfaces` skill for interface design and composition
- → See `go-testing` skill for testing with dependency injection
