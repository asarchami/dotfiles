...
→ See `go-spf13-viper` for the viper side of this integration.
...
- → See `go-cli` skill for general CLI architecture — project layout, exit codes, signal handling, I/O patterns
- → See `go-spf13-viper` skill for configuration layering alongside cobra (flag → env → file → default precedence)
