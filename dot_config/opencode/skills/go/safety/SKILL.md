...
For integer overflow as a security vulnerability, see the `go-security` skill section.
...
Many safety pitfalls are caught automatically by linters: `errcheck`, `forcetypeassert`, `nilerr`, `govet`, `staticcheck`. See the `go-lint` skill for configuration and usage.

## Cross-References

- → See `go-concurrency` skill for concurrent access patterns and sync primitives
- → See `go-data-structures` skill for slice/map internals, capacity growth, and container/ packages
- → See `go-error-handling` skill for nil error interface trap
- → See `go-security` skill for security-relevant safety issues (memory safety, integer overflow)
- → See `go-troubleshooting` skill for debugging panics and race conditions
