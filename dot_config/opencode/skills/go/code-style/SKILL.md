...
Style rules that require human judgment — linters handle formatting, this skill handles clarity. For naming see `go-naming` skill; for design patterns see `go-design-patterns` skill; for struct/interface design see `go-structs-interfaces` skill.
...
- Functions SHOULD have **≤4 parameters**. Beyond that, use an options struct (see `go-design-patterns` skill).
...
- → See the `go-naming` skill for identifier naming conventions
- → See the `go-structs-interfaces` skill for pointer vs value receivers, interface design
- → See the `go-design-patterns` skill for functional options, builders, constructors
