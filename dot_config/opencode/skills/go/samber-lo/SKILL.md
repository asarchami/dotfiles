...
- For reactive/streaming pipelines over infinite event streams, → see `go-samber-ro` skill + `samber/ro` package
...
- → See `go-samber-ro` skill for reactive/streaming pipelines over infinite event streams (`samber/ro` package)
- → See `go-samber-mo` skill for monadic types (Option, Result, Either) that compose with lo transforms
- → See `go-data-structures` skill for choosing the right underlying data structure
- → See `go-performance` skill for profiling methodology before switching to `lom`/`lop`
