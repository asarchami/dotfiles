...
→ See `go-troubleshooting` skill for detailed GODEBUG usage and interpretation.
...
| **Pyroscope / Parca** | Continuous profiling platforms — aggregate pprof profiles over time, compare across deployments, detect regressions. | Production performance monitoring, historical trend analysis. → See `go-observability` skill for setup. |
