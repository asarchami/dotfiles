...
- → See `go-samber-lo` skill for finite slice transforms (Map, Filter, Reduce, GroupBy) — use lo when data is already in a slice
- → See `go-samber-mo` skill for monadic types (Option, Result, Either) that compose with ro pipelines
- → See `go-samber-hot` skill for in-memory caching (also available as an ro plugin)
- → See `go-concurrency` skill for goroutine/channel patterns when reactive streams are overkill
- → See `go-observability` skill for monitoring reactive pipelines in production
